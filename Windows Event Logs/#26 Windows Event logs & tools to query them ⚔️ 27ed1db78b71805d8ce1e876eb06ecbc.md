# #26: Windows Event logs & tools to query them ⚔️
...
In this room, I explored **Windows Event Logs** in depth:

- Learned about **key Event IDs** and resources (MITRE, NSA, Microsoft docs).
- Practiced **PowerShell + XPath queries** using `Get-WinEvent` and Event Viewer.
- Detected attacks such as:
    - **PowerShell downgrade (Event ID 400)**
    - **Log clearing (Event ID 104)**
    - **Malware script execution (Event ID 4104)**
    - **Unauthorized group enumeration (Event ID 4799)**
- Understood the importance of enabling extra logging (PowerShell, Process Creation) for better visibility.

👉 This project showed me how to analyze `.evtx` files, write detection queries, and investigate adversary activity.
